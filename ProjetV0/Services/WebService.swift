        ...
        session.dataTask(with: url) { (data, response, error) in
            ...
            do {
                ...
                for propos in json.arrayValue {
                    ...
                    guard let contenu = propos["contenu"].string, let categorie = propos["categorie"]["contenu"].string, let likes = propos["likes"].int else {
                    ...
                    // Création du propos
                    let unPropos : Propos = Propos(contenu: contenu, categorie: categorie, createur: createur, likes: likes, reponses: reponses, commentaires: commentaires)
                    // Création des réponses associées au propos
                    for rep in propos["reponses"].arrayValue {
                        // Récupération des données d'une réponse
                        guard let contenu = rep["contenu"].string, let likes = rep["likes"].int, let dislikes = rep["dislikes"].int else {
                            print("Erreur dans la structure du JSON")
                            return
                        }
                        ...
                        let reponse = Reponse(contenu: contenu, categorie: "", createur: createur, propos: unPropos, likes: likes, dislikes: dislikes)
                        unPropos.reponses.append(reponse)
                    }
                    // Création des commentaires associés au propos
                    for com in propos["reponses"].arrayValue {
                        // Récupération des données d'un commentaire
                        guard let contenu = com["contenu"].string, let likes = com["likes"].int, let dislikes = com["dislikes"].int else {
                            print("Erreur dans la structure du JSON")
                            return
                        }
                        // Création du commentaire et ajout à la liste des commentaires du propos précédemment créé
                        let commentaire = Commentaire(contenu: contenu, createur: createur, propos: unPropos, likes: likes, dislikes: dislikes)
